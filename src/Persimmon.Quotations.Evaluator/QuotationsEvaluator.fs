...
module QuotationEvaluationTypes =
    ...
    let extraHashCompareType = This.Assembly.GetType("Persimmon.Quotations.Evaluator.ExtraHashCompare")
    ...
    type ConvEnv = {
        ...
        registeredValues  : ConcurrentDictionary<TestCaseTree, obj>
    }
...
//    | AsLetRecRecord of ParameterExpression * Expression * Expression
    ...
    and LetRecConvExpr (env:ConvEnv) (letrec:option<Var>) (inp:Expr) =
       //printf "ConvExpr : %A\n" e;
        ...
        | Patterns.Call(objOpt,minfo,args) ->
            ...
            | Λ ``-> getArray``  (_, [|ArrayTypeQ(_);_;_|],[x1;x2]) ->
            ...
            | _ ->
                let argsP = ConvExprs env args
                let instance = ConvObjArg env objOpt None
                ExpressionHelper.callAndRegisterResult env.registeredValues instance minfo argsP
                |> wrapVoid
    ...
    let Conv (e:#Expr, eraseEquality) =
        let e = optimize e
        let values = ConcurrentDictionary<TestCaseTree, obj>()
        let linqExpr = ConvExpr {
          eraseEquality = eraseEquality
          varEnv = Map.empty
          registeredValues = values } e
        (values, Expression.Lambda(linqExpr, Expression.Parameter(typeof<unit>)) |> asExpression)

    let CompileImpl (e: #Expr, eraseEquality) =
        let values, linqExpr = Conv (e,eraseEquality)
